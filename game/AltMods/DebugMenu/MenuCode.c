...
void DebugMenu_InitMenuPositionSize(struct DebugMenu *dm, int charSizeX, int charSizeY, int posX, int posY)
{
	int len = 0;
	int rowCount = 0;
	struct DebugRow *dr;
	...
	while (dr->actionFlag != 0)
	{
		if (dr->actionFlag == 1)
		{
			DebugMenu_InitMenuPositionSize(dr->subMenu, charSizeX, charSizeY, dm->posX + dm->sizeX + 10, posY);
		}
...
void DebugMenu_Init()
{
	DebugMenu_InitMenuPositionSize(&menuDbgMain, 8, 8, 0x14, 0x41);
}
...
void DebugMenu_Draw(struct DebugMenu *dm)
{
	...
	while (dr->actionFlag != 0)
	{
		...
		DebugFont_DrawLine(text, 3 + dm->posX, 3 + dm->posY + (rowCount * 8), color);
