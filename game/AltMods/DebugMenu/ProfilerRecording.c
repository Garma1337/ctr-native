...
void DebugProfiler_SectionStart(char *name, char r, char g, char b)
{
	...
	struct ProfilerSection *s = &ptrSectArr[numSectionsUsed];
...
void DebugProfiler_Draw()
{
	...
	for (int i = 0; i < numSectionsUsed; i++)
	{
		...
		if ((s->flagsVDT & 1) != 0)
		{
			DECOMP_DecalFont_DrawLine("V", DebugProfiler_Scale(s->posV), 0x2F, FONT_BIG, 0xffff8000);
		}

		if ((s->flagsVDT & 2) != 0)
		{
			DECOMP_DecalFont_DrawLine("D", DebugProfiler_Scale(s->posD), 0x2B, FONT_BIG, 0xffff8000);
		}
