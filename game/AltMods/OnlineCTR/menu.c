...
struct MenuRow menuRows[9] = {{0, 0, 1, 0, 0},
                              {0, 0, 2, 1, 1},
                              {0, 1, 3, 2, 2},
                              {0, 2, 4, 3, 3},
                              {0, 3, 5, 4, 4},
...
char *countryNames[8] = {
    "Europe", "USA NYC", "Mexico", "Brazil", "Australia", "Singapore", "Beta", "Private Room",
};
...
void NewPage_Tracks()
{
	...
	for (i = 0; i < 8; i++)
	{
		menuRows[i].stringIndex = data.metaDataLEV[8 * octr->PageNumber + i].name_LNG;
	}
}
...
void NewPage_Characters()
{
	...
	for (i = 0; i < 8; i++)
	{
		menuRows[i].stringIndex = data.MetaDataCharacters[8 * octr->PageNumber + i].name_LNG_long;
	}
}
...
void UpdateMenu()
{
	...
	if (buttons & BTN_LEFT)
	{
		octr->PageNumber = max(0, octr->PageNumber - 1);
	}
	if (buttons & BTN_RIGHT)
	{
		octr->PageNumber = min(pageMax, octr->PageNumber + 1);
	}
	...
	int string = // #/#\0  where '#' represents a single digit number
	    (('1' + octr->PageNumber) << 0) | ('/' << 8) | (('1' + pageMax) << 16);
...
void PrintCharacterStats()
{
	...
	DecalFont_DrawLine(countryNames[octr->serverCountry], 0x10, 0x10, FONT_SMALL, 0);

	char *roomName = "ROOM x";
	roomName[5] = GetRoomChar(octr->serverRoom + 1);

	DecalFont_DrawLine(roomName, 0x10, 0x18, FONT_SMALL, 0);
	...
	for (i = 0; i < octr->NumDrivers; i++)
	{
		...
		// 0x19 - red
		// 0x1A - green
		int color = octr->boolLockedInCharacters[i] ? PLAYER_GREEN : PLAYER_RED;
		...
		char *characterName = sdata->lngStrings[data.MetaDataCharacters[data.characterIDs[slot]].name_LNG_short];
...
void PrintRecvTrack()
{
	...
	sprintf(message, "Track: %s", sdata->lngStrings[data.metaDataLEV[octr->levelID].name_LNG]);
