...
void EndOfRace_Camera()
{
	...
	if (pad->buttonsTapped & BTN_R1)
	{
		SetNextCamera(true);
	}
	if (pad->buttonsTapped & BTN_L1)
	{
		SetNextCamera(false);
	}
}
...
void UpdateBestTimes()
{
	...
	for (int i = 0; i < octr->NumDrivers; i++)
	{
		...
		if ((octr->nameBuffer[index][0] == 0) || (octr->raceStats[i].finalTime == 0))
		{
			continue;
		}
...
void EndOfRace_Icons()
{
	...
	for (int i = 0; i < octr->NumDrivers; i++)
	{
		...
		else if (playersFinished > playersPerRow)
		{
			rectHeightMultiplier = 2;
		}
	}
